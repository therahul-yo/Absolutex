 ...
 *
 * Shared by the filesystem watcher (WatchService over configured roots) and the MediaStore
 * observer (the SAF-side complement for indexed shared storage): both emit this type, so the
 * repository consumes one event stream and no rival mapping can drift.
 */
