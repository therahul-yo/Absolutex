...
| Tap → first page < 250 ms | Warm, time to full display (base layer decoded): median 312.9 · min 266.7 · max 403.5 ms; window up at 78 ms | **not met** (was 513.6) |
...
Tap → first page went 513.6 → 373.3 → 312.9 ms at the median. First, neighbours stopped decoding alongside the
page being opened (three 6 MP JPEGs contended and the one on screen finished last). Then a trace showed the
opened page itself decoded twice at once, identical page, size and viewport; base layers are now shared, one
decode per page and size. What remains is ~75 ms of archive open and one ~175 ms decode, which is the next cut.
